...
export function LogViewer({ stationId }: { stationId: string }) {
  ...
  const load = useCallback(() => {
    apiFetch(`/stations/${stationId}/logs?limit=200`)
      .then((r) => r.json())
      .then((data: LogEntry[]) => {
        setLogs(data);
        if (!hovered.current) {
  ...
  const visible = filter === "ALL" ? logs : logs.filter((l) => l.level?.toUpperCase() === filter);
  ...
  return (
    <div className="flex flex-col h-full">
      <SectionHeader label={t("systemLog")}>
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            {POLL_OPTIONS.map((s) => (
              <button key={s} onClick={() => setIntervalVal(s)}
                className={["text-[11px] font-mono px-1.5 py-0.5 rounded transition-colors",
                  interval === s && !paused ? "bg-accent-dim text-accent" : "text-text-muted hover:text-text",
                ].join(" ")}>
                {s}s
              </button>
            ))}
          ...
          <button onClick={() => setPaused((p) => !p)}
            className={["text-[11px] font-mono px-1.5 py-0.5 rounded border transition-colors",
              paused ? "border-yellow-400/40 bg-yellow-400/10 text-yellow-400" : "border-border text-text-muted hover:text-text",
            ].join(" ")}>
      ...
      <div className="flex-1 overflow-y-auto font-mono text-[11.5px] bg-bg rounded-lg border border-border p-2 min-h-0"
        ...
        {visible.length === 0 ? (
          ...
          visible.map((entry, i) => {
            ...
            return (
              <div key={i} className="flex gap-2 py-px hover:bg-surface-hi px-1 rounded">
