...
export default function ImportPanel({
  ...
  onOverlay: (files: File[], name: string) => void;
}) {
  ...
  const canSend =
    files.length > 0 && observationSet.trim() !== "" && status === "idle";
  ...
  return (
    <div className="bg-surface border border-border rounded-[14px] overflow-hidden flex flex-col h-full">
      ...
      <div className="p-6 flex flex-col gap-5 flex-1">
        ...
        <div className="flex flex-col gap-1.5">
          <Label>
            {files.length ? t("filesCount", { count: files.length }) : ""}
          </Label>
          <div
            ...
            {files.length > 0 ? (
              <div className="w-full flex flex-col gap-2 max-h-[200px] overflow-y-auto p-1">
                {files.map((f, i) => (
                  <div
                    key={`${f.name}-${i}`}
                    className="flex items-center gap-2 px-3 py-2 bg-surface border border-border rounded-md group"
                  >
