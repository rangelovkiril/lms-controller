...
const app = new Elysia()
  ...
  .onStart(({ decorator: { mqtt, influx }, server }) => {
    cmdRef.handle = (stationId, action, objId) => {
      ...
      console.log(
        `[CMD] Published "${action}" for obj="${objId}" → slr/${stationId}/cmd`,
      )
    }
  ...
  .group("/api", (app) =>
    app
      ...
      .post("/stations", ({ influx, body }) => createStation(influx, body), {
        body: t.Object({
      ...
      .get(
        ...
        async ({ influx, params, error }) => {
          ...
          if (!meta)
            return error(404, { message: `Station "${params.id}" not found` })
          return meta
        },
