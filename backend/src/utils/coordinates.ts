...
export function toCartesial(r: number, theta: number, phi: number, useDegrees: boolean = false): {x: number, y: number, z: number}{
  const t = useDegrees ? theta * (Math.PI / 180) : theta
  const p = useDegrees ? theta * (Math.PI / 180) : theta
